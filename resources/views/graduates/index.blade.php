    ...
    <div class="dashboard-wrap graduate-wrap" x-data="{ search: '', status: '{{ request('status', 'all') }}', year: 'all', campus: 'all', school: 'all', major: 'all', view: 'grid' }">
        ...
        <section class="filter-ribbon" aria-label="Graduate profile filters">
            <div class="filter-ribbon-heading">
                <div>
                    <p class="eyebrow">Filter profiles</p><span>Focus the directory by edition, place, or discipline.</span>
                </div><a href="{{ route('graduates.index') }}" class="clear-filters" x-show="status !== 'all' || year !== 'all' || campus !== 'all' || school !== 'all' || major !== 'all'">Clear filters</a>
            </div>
            ...
            <div class="ribbon-fields">
                <select class="directory-filter" x-model="year" aria-label="Filter by academic year">
                    <option value="all">All years</option>@foreach ($academicYears as $academicYear)<option value="{{ $academicYear->id }}">{{ $academicYear->title }}</option>@endforeach
                </select>
                <select class="directory-filter" x-model="campus" aria-label="Filter by campus">
                    <option value="all">All campuses</option>@foreach ($campuses as $campus)<option value="{{ $campus->id }}">{{ $campus->name }}</option>@endforeach
                </select>
                <select class="directory-filter" x-model="school" aria-label="Filter by school">
                    <option value="all">All schools</option>@foreach ($schools as $school)<option value="{{ $school->id }}">{{ $school->name }}</option>@endforeach
                </select>
                <select class="directory-filter" x-model="major" aria-label="Filter by major">
                    <option value="all">All majors</option>@foreach ($majors as $major)<option value="{{ $major->id }}">{{ $major->name }}</option>@endforeach
                </select>
            </div>
        </section>
        ...
        <section class="graduate-toolbar" aria-label="Graduate profile tools">
            ...
            <div class="graduate-toolbar-tools">
                <div class="view-switcher" aria-label="Profile view"><button type="button" class="view-button" :class="{ 'is-selected': view === 'grid' }" @click="view = 'grid'" aria-label="Grid view" title="Grid view"><span aria-hidden="true">▦</span></button><button type="button" class="view-button" :class="{ 'is-selected': view === 'list' }" @click="view = 'list'" aria-label="List view" title="List view"><span aria-hidden="true">☷</span></button></div>
            </div>
        </section>
        ...
        <section class="graduate-section">
            <div class="section-heading">
                ...
                </div><span class="panel-meta">{{ $graduates->total() }} records</span>
            </div>
            <div class="graduate-grid" :class="{ 'graduate-list': view === 'list' }">
                ...
                <article class="graduate-card" x-show="(status === 'all' || status === '{{ $graduate->publish_status }}') && (year === 'all' || year === '{{ $graduate->graduation?->academic_year_id }}') && (campus === 'all' || campus === '{{ $graduate->campus_id }}') && (school === 'all' || school === '{{ $graduate->school_id }}') && (major === 'all' || major === '{{ $graduate->major_id }}') && @js($searchText).includes(search.toLowerCase())">
                    <div class="graduate-card-top">@if ($graduate->portraitMedia)<img class="graduate-avatar graduate-avatar-photo" src="{{ Storage::disk('public')->url($graduate->portraitMedia->path) }}" alt="{{ $graduate->portraitMedia->alt_text ?? $graduate->name }}">@else<div class="graduate-avatar">{{ $initials }}</div>@endif<span class="edition-status {{ $graduate->publish_status === 'published' ? 'is-active' : 'is-archived' }}">{{ ucfirst($graduate->publish_status) }}</span></div>
                    <h3>{{ $graduate->name }}</h3>
                ...
                <div class="empty-editions">
                    ...
                    <p>Add a graduate to begin shaping this year's collection.</p><a href="{{ route('graduates.create') }}" class="button button-navy">Add graduate <span aria-hidden="true">→</span></a>
                </div>
                @endforelse
            </div>
