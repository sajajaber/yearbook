...
<x-app-layout>
    <x-slot name="header">
        <div class="dashboard-heading graduate-heading">
            ...
            </div><a href="{{ route('graduates.index') }}" class="text-link">Back to profiles <span aria-hidden="true">←</span></a>
        </div>
    </x-slot>
            ...
            <div class="form-main">
                <section class="form-section">
                    ...
                    <div class="form-grid form-grid-two">
                        ...
                        <label class="form-field"><span>GPA</span><input type="number" name="gpa" value="{{ old('gpa', $graduate->gpa ?? '') }}" min="0" max="4" step="0.01" placeholder="e.g. 3.72">@error('gpa')<small>{{ $message }}</small>@enderror</label>
                        ...
                        <label class="form-field"><span>Academic year</span><select name="academic_year_id" id="academic_year_id" required>
                                <option value="">Select academic year</option>@foreach ($availableAcademicYears as $academicYear)<option value="{{ $academicYear->id }}" @selected(old('academic_year_id', $graduate->academic_year_id ?? ($activeAcademicYear?->id ?? '')) == $academicYear->id)>{{ $academicYear->title }}{{ $academicYear->status === 'active' ? ' · Active' : ($academicYear->status === 'archived' ? ' · Archived' : '') }}</option>@endforeach
                            </select>@error('academic_year_id')<small>{{ $message }}</small>@enderror</label>
                        <label class="form-field"><span>Ceremony attendance</span><select name="graduation_id" id="graduation_id">
                                <option value="">Not attending a ceremony</option>@foreach ($graduations as $graduation)<option value="{{ $graduation->id }}" data-academic-year-id="{{ $graduation->academic_year_id }}" @selected(old('graduation_id', $graduate->graduation_id ?? '') == $graduation->id)>{{ $graduation->academicYear?->title ?? 'Edition' }} · {{ $graduation->venue ?: 'Ceremony' }}</option>@endforeach
            ...
            <aside class="form-aside">
                ...
                <section class="form-section form-section-compact">
                    <div class="form-section-heading">
                        ...
                        <h2>{{ $isEdit && $graduate->resumeMedia ? 'Update resume / CV' : 'Add resume / CV' }}</h2>
                    </div>
                    @if ($isEdit && $graduate->resumeMedia)
                    <div class="upload-note"><strong>{{ $graduate->resumeMedia->file_name }}</strong><br><span>PDF currently attached to this graduate.</span></div>
                    @else
                    <p class="upload-note">Upload the graduate's resume as a PDF. It will be public with the graduate's published profile.</p>
                    @endif
                    <label class="upload-field"><span>{{ $isEdit && $graduate->resumeMedia ? 'Choose a replacement PDF' : 'Choose a resume PDF' }}</span><input type="file" name="resume" accept="application/pdf"><small>PDF only. Maximum 10 MB.</small></label>
                    @error('resume')<small class="form-error">{{ $message }}</small>@enderror
                </section>
                ...
                <section class="form-section form-section-compact">
                    <div class="form-section-heading">
                        ...
                        <h2>Permissions</h2>
                            ...
                            <option value="reviewed" @selected(old('publish_status', $graduate->publish_status ?? '') === 'reviewed')>Submitted for review</option>
                            ...
                            <option value="rejected" @selected(old('publish_status', $graduate->publish_status ?? '') === 'rejected')>Changes requested</option>
    ...
    <script>
        ...
        function filterGraduations() {
            ...
            const currentValue = graduationSelect.value;
            graduationSelect.innerHTML = '';
            graduationSelect.appendChild(allGraduationOptions[0]);
            allGraduationOptions.slice(1).forEach(option => {
                if (option.dataset.academicYearId === selectedYearId) graduationSelect.appendChild(option);
            });
            graduationSelect.value = Array.from(graduationSelect.options).some(option => option.value === currentValue) ? currentValue : '';
        }
        schoolSelect.addEventListener('change', filterMajors);
        academicYearSelect.addEventListener('change', filterGraduations);
        filterMajors();
        filterGraduations();
    </script>
</x-app-layout>