<x-app-layout>
    ...
    <div class="dashboard-wrap" style="max-width: 980px; margin: 0 auto;">
        <style>
            ...
            .media-edit-assignment select:focus {
                outline: 3px solid rgba(255, 176, 52, .18);
                border-color: #ffb034;
            }

            .media-edit-assignment {
                margin-top: 24px;
                padding: 20px;
                border: 1px solid #d8e3ef;
                border-radius: 16px;
            ...
            .media-ai-button:hover {
                border-color: #ffb034;
                background: #fffaf0;
            }

            .media-ai-button:disabled {
                opacity: .6;
                cursor: wait;
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function addTag(value = input.value.trim()) {
                ...
                if (values.length >= 30) {
                    aiStatus && (aiStatus.textContent = 'You can add a maximum of 30 tags.');
                    return;
                }
                ...
                remove.style.cssText = 'margin-left:5px;border:0;background:transparent;color:inherit;cursor:pointer;font-weight:800;';
                ...
                hidden.name = 'tags[]';
                hidden.value = value;
                wrapper.append(text, remove, hidden);
            ...
            document.getElementById('generate-caption')?.addEventListener('click', () => runAi('{{ route('
                media.generate - caption ', $mediaItem->id) }}', 'caption'));
            document.getElementById('generate-tags')?.addEventListener('click', () => runAi('{{ route('
                media.generate - tags ', $mediaItem->id) }}', 'tags'));
        });
    </script>
</x-app-layout>