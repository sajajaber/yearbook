<x-app-layout>
    ...
    <div class="dashboard-wrap" style="max-width: 820px; margin: 0 auto;">
        ...
        <form id="media-upload-form" action="{{ route('media.store') }}" method="POST" enctype="multipart/form-data" class="media-create-card">
            ...
            <div id="media-ai-box" class="media-ai-box" hidden aria-live="polite">
                <div class="media-ai-head"><p class="media-ai-title">✨ AI editorial assistance</p><span style="font-size:.7rem;color:#64748b;">Images only</span></div>
                <div id="media-ai-status" class="media-ai-status">Select an image to generate a suggested caption and searchable tags. Nothing is saved automatically.</div>
            </div>
            ...
            <div class="media-create-field">
                ...
                <small id="tag-help" class="media-create-help">Tags are optional. You can edit or remove AI suggestions before uploading. Press Enter or click Add tag to add one.</small>
    ...
    <div id="compression-modal" hidden aria-hidden="true" style="position:fixed;inset:0;z-index:9999;align-items:center;justify-content:center;padding:20px;background:rgba(0,20,45,.62)">
        <div role="dialog" aria-modal="true" aria-labelledby="compression-title" style="width:min(100%,520px);background:#fff;border-radius:18px;box-shadow:0 24px 70px rgba(0,0,0,.22);overflow:hidden">
            ...
            <div style="padding:22px 26px;color:#475569;line-height:1.6"><p>The selected image is larger than the recommended upload size. It will be compressed before uploading. Compression may slightly reduce image quality.</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><div><small>Original size</small><strong id="compression-original-size" style="display:block;color:#002a5c"></strong></div><div><small>Estimated compressed size</small><strong id="compression-estimated-size" style="display:block;color:#002a5c"></strong></div></div><p id="compression-status" hidden aria-live="polite" style="font-weight:700;color:#002a5c"></p></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const aiBox=document.getElementById('media-ai-box');
            ...
            function addTagValue(value){
                ...
                wrapper.append(text,remove,hidden);tagList.appendChild(wrapper);
            }
            function addTag(){const value=tagInput.value.trim();if(!value)return;const before=tagList.querySelectorAll('input[name="tags[]"]').length;addTagValue(value);if(tagList.querySelectorAll('input[name="tags[]"]').length>before){tagInput.value='';tagInput.focus();}else if(before>=30){showError('You can add a maximum of 30 tags to one media item.');}}
            ...
            function rejectImageCompression(){closeCompressionModal();compressionAccepted=false;fileInput.value='';aiBox.hidden=true;help.textContent='Images over 5 MB require compression confirmation before upload.';showError('The image was not uploaded because compression was rejected.');}
            ...
            async function requestAiSuggestions(file){
                if(!file||getType(file)!=='image')return;
                const token=++aiRequestToken;
                aiBox.hidden=false;aiStatus.className='media-ai-status';aiStatus.textContent='Analyzing image for a suggested caption and searchable tags…';
                const data=new FormData();data.append('file',file);
                try{
                    const response=await fetch('{{ route('media.ai-suggestions') }}',{method:'POST',body:data,headers:{'X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]')?.getAttribute('content')||'{{ csrf_token() }}','Accept':'application/json','X-Requested-With':'XMLHttpRequest'},credentials:'same-origin'});
                    const payload=await response.json();
                    if(token!==aiRequestToken)return;
                    if(!response.ok)throw new Error(payload.message||'AI suggestions could not be generated.');
                    if(payload.caption){document.getElementById('caption').value=payload.caption;}
                    if(Array.isArray(payload.tags)){payload.tags.forEach(addTagValue);}
                    aiStatus.className='media-ai-status is-success';aiStatus.textContent='Suggestions added. Review or edit the caption and tags before uploading. Nothing was saved automatically.';
                }catch(error){
                    if(token!==aiRequestToken)return;
                    aiStatus.className='media-ai-status is-error';aiStatus.textContent=`AI assistance was unavailable: ${error.message} You can still enter the caption and tags manually.`;
                }
            }
            function prepareSelectedImage(file){compressionAccepted=true;help.textContent='Image ready. AI suggestions can be edited before upload.';requestAiSuggestions(file);}

            fileInput.addEventListener('change',function(){
                clearError();compressionAccepted=false;aiRequestToken++;aiBox.hidden=true;
                const file=fileInput.files[0];if(!file)return;
                const type=getType(file);
                if(!type){showError('Unsupported file type. Please choose an accepted image, video, or document.');fileInput.value='';return;}
                if(file.size>limits[type]){showError(`${type.charAt(0).toUpperCase()+type.slice(1)} exceeds the allowed upload size.`);fileInput.value='';return;}
                if(type==='image'){
                    if(file.size>imageCompressionThreshold){openCompressionModal(file);}else{prepareSelectedImage(file);}
                }
            });

            acceptButton.addEventListener('click',async function(){
                if(!pendingImage)return;acceptButton.disabled=true;rejectButton.disabled=true;compressionStatus.hidden=false;compressionStatus.textContent='Compressing image…';
                try{
                    const blob=await compressImage(pendingImage);
                    if(blob.size>limits.image)throw new Error('The compressed image is still larger than the 10 MB limit. Please choose a smaller image.');
                    if(!replaceSelectedFile(blob,pendingImage))throw new Error('Your browser could not prepare the compressed image for upload.');
                    const selected=fileInput.files[0];compressionAccepted=true;help.textContent=`Compressed image ready: ${formatBytes(blob.size)}. Quality may be slightly reduced.`;compressionStatus.textContent='Compression complete. Analyzing the image…';closeCompressionModal();prepareSelectedImage(selected);
                }catch(error){compressionStatus.textContent=error.message;acceptButton.disabled=false;rejectButton.disabled=false;}
            });
