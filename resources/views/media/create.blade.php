    ...
    <div class="dashboard-wrap" style="max-width:820px;margin:0 auto;">
        <style>
            .media-create-card {
                ...
                border-radius: 22px;
                background: #fff;
                box-shadow: 0 10px 35px rgba(0, 42, 92, .06)
            }
            ...
            .media-ai-inline:hover {
                border-color: #ffb034;
                background: #fffaf2
            }

            .media-ai-inline:disabled {
                opacity: .55;
                cursor: wait
            }

            .media-create-field input,
            .media-create-field textarea {
                width: 100%;
            ...
            .media-create-field input:focus,
            .media-create-field textarea:focus {
            ...
            .media-ai-status.is-error {
                color: #8b1e1e
            }

            .media-ai-status.is-success {
                color: #17663a
            }
        ...
        @if($errors->any())<div class="media-alert media-alert-error" role="alert"><strong>Please fix the following errors:</strong>
    ...
    <div id="compression-modal" class="compression-modal" hidden aria-hidden="true">
        <div class="compression-dialog" role="dialog" aria-modal="true" aria-labelledby="compression-title">
            <div class="compression-head">
                <h3 id="compression-title">This image is large</h3>
            </div>
            <div class="compression-body">
                ...
                <div class="compression-stats">
                    <div><small>Original size</small><strong id="compression-original-size"></strong></div>
                    <div><small>Estimated compressed size</small><strong id="compression-estimated-size"></strong></div>
                </div>
                <p id="compression-status" hidden aria-live="polite"></p>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const form = document.getElementById('media-upload-form'),
                ...
                errorBox = document.getElementById('client-upload-error'),
                help = document.getElementById('file-help'),
            ...
            const modal = document.getElementById('compression-modal'),
                ...
                reject = document.getElementById('compression-reject'),
                originalSize = document.getElementById('compression-original-size'),
                estimatedSize = document.getElementById('compression-estimated-size');
            const limits = {
                    image: 10 * 1024 * 1024,
                    video: 100 * 1024 * 1024,
            ...
            function addTagValue(value) {
                ...
                if (!value) return;
                const existing = [...tagList.querySelectorAll('input[name="tags[]"]')].map(i => i.value.toLowerCase());
                if (existing.includes(value.toLowerCase()) || existing.length >= 30) return;
            ...
            function addTag() {
                ...
                const before = tagList.querySelectorAll('input[name="tags[]"]').length;
                addTagValue(v);
                if (tagList.querySelectorAll('input[name="tags[]"]').length > before) {
                    tagInput.value = '';
                    tagInput.focus()
                }
            }
            ...
            function compress(file) {
                return new Promise((resolve, reject) => {
                    ...
                    img.onload = () => {
                        ...
                        if (!ctx) return reject(new Error('Browser compression is unavailable.'));
                        ctx.drawImage(img, 0, 0, w, h);
                        canvas.toBlob(b => b ? resolve(b) : reject(new Error('Image compression failed.')), 'image/jpeg', .84)
                    };
                    img.onerror = () => {
                        URL.revokeObjectURL(url);
            ...
            accept.onclick = async () => {
                ...
                try {
                    ...
                    const compressed = new File([blob], `${file.name.replace(/\.[^.]+$/,'')}-compressed.jpg`, {
                        type: 'image/jpeg'
                    });
