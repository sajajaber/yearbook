...
@section('extra-css')
<style>
    .events-page { --e-ink:var(--ink,#002a5c); --e-gold:var(--red,#ffb034); --e-muted:#64748b; --e-line:#d8e3ef; --e-ease:cubic-bezier(.16,1,.3,1); background:#f7fbff; }
    ...
    .events-hero-inner { position:relative; z-index:2; width:100%; max-width:1180px; margin:0 auto; }
    ...
    .events-page > main.container { max-width:1240px; padding-left:32px; padding-right:32px; }
    ...
    .events-section-intro { display:flex; align-items:end; justify-content:space-between; gap:20px; margin-bottom:22px; }
    ...
    .events-list { display:flex; flex-direction:column; gap:12px; border-top:0; }
    .event-list-link { display:block; color:inherit; text-decoration:none; }
    .event-list-item { display:grid; grid-template-columns:135px minmax(0,1fr) 42px; gap:28px; align-items:center; min-height:142px; padding:25px 28px; border:1px solid var(--e-line); border-radius:18px; background:#fff; box-shadow:0 5px 20px rgba(15,23,42,.035); transition:transform .45s var(--e-ease),box-shadow .45s ease,border-color .3s ease; }
    .event-list-link:hover .event-list-item { transform:translateY(-4px); border-color:rgba(180,135,56,.42); box-shadow:0 18px 38px rgba(15,23,42,.075); }
    ...
    .event-list-arrow { display:flex; align-items:center; justify-content:center; width:36px; height:36px; border:1px solid var(--e-line); border-radius:50%; color:var(--e-ink); transition:all .35s var(--e-ease); }
    .event-list-link:hover .event-list-arrow { border-color:#c8a15b; background:#fff8ea; transform:translateX(4px) rotate(-4deg); }
    .events-list.view-cards { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:18px; }
    .events-list.view-cards .event-list-item { height:100%; min-height:285px; grid-template-columns:minmax(0,1fr) 38px; grid-template-rows:auto 1fr; align-items:start; gap:18px; padding:28px; }
    .events-list.view-cards .event-list-date { grid-column:1/-1; }
    .events-list.view-cards .event-list-main { grid-column:1; }
    .events-list.view-cards .event-list-arrow { grid-column:2; grid-row:2; }
    .events-list.view-cards .event-list-link:hover .event-list-item { transform:translateY(-5px); }
    ...
    @media(max-width:850px){ .events-page > main.container{padding-left:24px;padding-right:24px;} .events-hero{min-height:520px;padding:78px 6vw 65px;} .events-hero-bottom{align-items:flex-start;flex-direction:column;gap:22px;} .events-hero-count{text-align:left;} .events-list.view-cards{grid-template-columns:1fr;} }
    @media(max-width:650px){ .events-page > main.container{padding-left:18px;padding-right:18px;} .events-hero{min-height:500px;padding:62px 24px 52px;} .events-hero h1{letter-spacing:-2.5px;} .events-filter-form{grid-template-columns:1fr 1fr;} .events-section-intro{align-items:flex-start;flex-direction:column;} .events-view-tools{align-self:flex-end;} .event-list-item{grid-template-columns:88px minmax(0,1fr) 30px;gap:15px;padding:20px;} .events-list.view-cards .event-list-item{grid-template-columns:minmax(0,1fr) 30px;padding:23px;} }
    @media(max-width:430px){ .events-filter-form{grid-template-columns:1fr;} .event-list-item{grid-template-columns:1fr 30px;} .event-list-date{grid-column:1/-1;} }
...
<div class="events-page">
    <section class="events-hero">
        <div class="events-hero-grid"></div><div class="events-hero-glow"></div>
        <div class="container events-hero-inner">
            <div class="events-kicker">Yearbook Journal</div>
            <h1>Moments<br><em>that mattered.</em></h1>
            <div class="events-hero-bottom"><p class="events-hero-copy">A curated record of the gatherings, milestones, celebrations and stories that shaped the academic year.</p><div class="events-hero-count"><strong>{{ $events->total() }}</strong><span>archived events</span></div></div>
        </div>
    </section>
    <main class="container">
        <section class="events-discovery"><div class="events-filter-shell"><form method="GET" action="{{ route('public.events') }}" class="events-filter-form">
        ...
        <section><div class="events-section-intro"><div><div class="events-section-label">The archive</div><h2>Events &amp; experiences</h2></div><div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;justify-content:flex-end;"><span class="events-results">{{ $events->total() }} {{ Str::plural('event', $events->total()) }}</span><div class="events-view-tools" aria-label="Choose event view"><button type="button" class="events-view-button is-active" data-events-view="list">List</button><button type="button" class="events-view-button" data-events-view="cards">Cards</button></div></div></div>
            @if($events->count())
                <div class="events-list" id="events-list">
                    @foreach($events as $event)
                        <a class="event-list-link" href="{{ route('public.event.detail', $event->id) }}"><article class="event-list-item"><div class="event-list-date">{{ optional($event->event_date)->format('d M Y') ?? \Carbon\Carbon::parse($event->event_date)->format('d M Y') }}</div><div class="event-list-main"><div class="event-list-meta">@if($event->category)<span class="event-category">{{ $event->category->name }}</span>@endif @if($event->location)<span class="event-list-location">{{ $event->location }}</span>@endif</div><h3 class="event-list-title">{{ $event->title }}</h3>@if($event->description)<p class="event-list-description">{{ Str::limit(strip_tags(\App\Support\RichText::sanitize($event->description)), 190) }}</p>@endif</div><span class="event-list-arrow" aria-hidden="true">→</span></article></a>
                    @endforeach
                </div>
