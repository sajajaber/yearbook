...
<style>
    ...
    .graduates-hero::after {
        ...
        opacity: 0;
        animation:
            graduatesCircleIn 1.4s var(--g-ease) .15s forwards,
            graduatesFloat 10s ease-in-out 1.8s infinite;
    }
    ...
    .graduates-hero-count {
        flex-shrink: 0;
        text-align: right;
    }

    .graduates-hero-count strong {
        display: block;
        color: #fff;
        font-size: 44px;
        line-height: 1;
    ...
    .graduates-filter-form {
        ...
        grid-template-columns:
            minmax(190px, 1.5fr) repeat(4, minmax(115px, 1fr)) auto;
        gap: 7px;
    }
    ...
    .graduates-filter-field select:focus {
        ...
        border-color: #d8e1ea;
        background: #fff;
        box-shadow: 0 0 0 3px rgba(215, 173, 89, .08);
    }
     ...
     * ---------------------------------------------------------
     *
     * These entries are intentionally NOT links and contain
     * no photo, profile information, GPA, resume, etc.
     */
...
/*
* ---------------------------------------------------------
* PREPARE PUBLIC DIRECTORY DATA
* ---------------------------------------------------------
*
* $graduates contains only graduates with granted consent
* and is paginated by the controller.
*
* $namedOnly contains pending/declined graduates.
*
* We combine them here for DIRECTORY STRUCTURE ONLY.
* The two types are still rendered completely differently.
*/

$visibleGrouped = $graduates->getCollection()
...
/*
* Create one unified degree/school structure.
*
* A school is included even if it currently contains only
* pending/declined graduates.
*/
$allDegreeLevels = collect(['undergraduate', 'graduate']);
...
$directoryTotal = $graduates->total() + $namedOnly->count();
            ...
            <div class="graduates-hero-content">
                ...
                <div class="graduates-hero-bottom">
                    ...
                    <p class="graduates-hero-copy">
                        Explore the graduating community by degree level,
                        school, academic year and campus. Each profile is
                        part of the yearbook record.
                    </p>
                ...
                <form
                    ...
                    <div class="graduates-filter-field">
                        ...
                        <select name="year">
                            ...
                            <option
                                ...
                                @selected((string) $year===(string) $academicYear->id)
                                >
                                {{ $academicYear->title }}
                                {{ $academicYear->status === 'active' ? ' · Current' : '' }}
                            </option>
                    ...
                    <div class="graduates-filter-field">
                        ...
                        <select name="degree">
                            ...
                            <option
                                value="undergraduate"
                                @selected($degree==='undergraduate' )>
                                Undergraduate
                            </option>

                            <option
                                value="graduate"
                                @selected($degree==='graduate' )>
                                Graduate
                            </option>
                    ...
                    <div class="graduates-filter-field">
                        ...
                        <select name="school">
                            ...
                            <option
                                value="{{ $item->id }}"
                                @selected((string) $school===(string) $item->id)
                                >
                                {{ $item->name }}
                            </option>
                    ...
                    <div class="graduates-filter-field">
                        ...
                        <select name="campus">
                            ...
                            <option
                                value="{{ $item->id }}"
                                @selected((string) $campus===(string) $item->id)
                                >
                                {{ $item->name }}
                            </option>
        ...
        <section>
            ...
            <div class="graduates-header">
                ...
                <div>
                    ...
                    <div class="graduates-kicker-small">
                        The directory
                    </div>
                ...
                <span class="graduates-count">
                    {{ $directoryTotal }}
                    {{ Str::plural('graduate', $directoryTotal) }}
                </span>
            ...
            @php
            $visibleSchools = $visibleGrouped->get(
            $degreeLevel,
            collect()
            );

            $namedSchools = $namedGrouped->get(
            $degreeLevel,
            collect()
            );

            /*
            * Merge school names from both collections.
            */
            $schoolNames = $visibleSchools
            ->keys()
            ->merge($namedSchools->keys())
            ->unique()
            ->sort()
            ->values();
            @endphp
            ...
            @php
            $degreeVisibleCount = $visibleSchools->flatten(1)->count();
            $degreeNamedCount = $namedSchools->flatten(1)->count();
            $degreeTotalCount = $degreeVisibleCount + $degreeNamedCount;
            @endphp
            ...
            <section class="degree-section">
                ...
                <div class="degree-heading">
                    ...
                    <span class="degree-count">
                        {{ $degreeTotalCount }}
                        {{ Str::plural('graduate', $degreeTotalCount) }}
                    </span>
                ...
                @php
                $schoolVisible = $visibleSchools->get(
                $schoolName,
                collect()
                );

                $schoolNamed = $namedSchools->get(
                $schoolName,
                collect()
                );
                ...
                <section class="school-section">
                    ...
                    <div class="school-heading">
                        ...
                        <span class="school-count">
                            {{ $schoolTotal }}
                            {{ Str::plural('graduate', $schoolTotal) }}
                        </span>
                    ...
                    {{-- =====================================================
                                     CONSENTED GRADUATES
                                     Full public cards and clickable profiles.
                                     ===================================================== --}}

                    @if($schoolVisible->isNotEmpty())

                    <div class="graduates-grid">

                        @foreach($schoolVisible as $graduate)

                        @php
                        $portrait =
                        $graduate->portraitMedia
                        ?? $graduate->media->firstWhere('type', 'image');

                        $graduateYear =
                        $graduate->academicYear?->title
                        ?? $graduate->graduation?->academicYear?->title;
                        @endphp
                        ...
                        <a
                            ...
                            <article class="graduate-card">
                                ...
                                <div class="graduate-portrait">
                                    ...
                                    <span class="graduate-placeholder">
                                        {{ mb_strtoupper(mb_substr($graduate->name, 0, 1)) }}
                                    </span>
                                    ...
                                    <span class="graduate-badge graduate-year-badge">
                                        {{ $graduateYear }}
                                    </span>
                    ...
                    {{-- =====================================================
                                     ...
                                     Name only. No links. No photos. No details.
                                     ===================================================== --}}

                    @if($schoolNamed->isNotEmpty())

                    <div
                        class="graduate-name-only-list"
                        aria-label="Graduates listed by name only">

                        @foreach($schoolNamed as $graduate)

                        <span class="graduate-name-only">
                            {{ $graduate->name }}
                        </span>
            ...
            {{-- =========================================================
                 PAGINATION
                 Only full public profiles are paginated.
                 Name-only graduates remain visible regardless of page.
                 ========================================================= --}}
            ...
            <div class="graduates-pagination">
                ...
                {!! $graduates
                ->onEachSide(1)
                ->links('pagination::simple-tailwind') !!}
            ...
            <div class="graduates-empty">
                ...
                <p>
                    Try another academic year, degree level,
                    school, campus or search term.
                </p>
