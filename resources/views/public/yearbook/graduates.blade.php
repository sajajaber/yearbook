...
<style>
    ...
    .graduate-card-body { padding: 17px 17px 18px; }
    ...
    @media (max-width:850px) {
        .graduates-hero { min-height: 530px; padding: 72px 6vw 70px; }
    ...
    @media (max-width:650px) {
        ...
        .graduates-hero { min-height: 510px; padding: 60px 24px 62px; }
    ...
    @media (prefers-reduced-motion: reduce) {
        .graduates-page * { animation: none !important; transition: none !important; }
        .graduate-link:hover .graduate-card,
        .graduate-link:focus-visible .graduate-card,
        .graduate-link:hover .graduate-portrait img,
...
@php
    ...
    $visibleUndergraduates = $graduates->getCollection()->where('degree_level', 'undergraduate')->count();
    $visibleGraduates = $graduates->getCollection()->where('degree_level', 'graduate')->count();
    ...
    <main class="container">
        ...
        <section class="graduates-overview" aria-label="Directory overview">
            ...
            <div class="directory-stat">
                ...
                <strong>{{ $graduates->getCollection()->where('degree_level', 'undergraduate')->count() + $namedOnly->where('degree_level', 'undergraduate')->count() }}</strong>
                <span>directory entries</span>
            </div>

            <div class="directory-stat">
                <small>Graduate</small>
                <strong>{{ $graduates->getCollection()->where('degree_level', 'graduate')->count() + $namedOnly->where('degree_level', 'graduate')->count() }}</strong>
                <span>directory entries</span>
            </div>
        </section>
            ...
            <section class="school-directory">
                <div class="school-directory-heading">
                    ...
                    <span class="directory-result-count">
                        {{ $directoryTotal }} {{ Str::plural('entry', $directoryTotal) }}
                        @if($graduates->hasPages()) · page {{ $graduates->currentPage() }} of {{ $graduates->lastPage() }} @endif
                    </span>
                </div>
