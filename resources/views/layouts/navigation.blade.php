<nav x-data="{ open: false }" class="site-nav">
                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            <div class="user-name">
                                {{ Auth::user()?->name }}
                                @if (Auth::user()?->role)
                                    <small>{{ ucfirst(Auth::user()?->role->role_name) }}</small>
                                @endif
                            </div>
                            <div class="ms-1">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="mobile-menu hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (in_array(Auth::user()?->role?->role_name, ['admin', 'reviewer']))
                <a href="{{ route('ai-generations.index') }}" class="{{ request()->routeIs('ai-generations.*') ? 'is-active' : '' }}">AI review</a>
            @endif
            @if (Auth::user()?->role?->role_name === 'admin')
                <a href="{{ route('settings.index') }}" class="{{ request()->routeIs('settings.*') ? 'is-active' : '' }}">Settings</a>
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4">
                ...
                @if (Auth::user()?->role)
                    <div class="mobile-role">{{ ucfirst(Auth::user()->role->role_name) }}</div>
                @endif
            </div>
    ...
    <style>
        ...
        @media (max-width: 639px) {
            .site-nav .flex.justify-between.h-16 {
                width: 100%;
                height: auto;
                min-height: 58px;
            }
