...
class GeminiVisionService
{
    ...
    public function analyzeImage(string $imageBase64, string $mimeType, string $prompt): string
    {
        if ($this->apiKey === '') {
            throw new RuntimeException('Gemini AI is not configured. Set GEMINI_API_KEY in the environment.');
        }

        if ($imageBase64 === '' || trim($prompt) === '') {
            throw new RuntimeException('Gemini AI received incomplete image analysis data.');
        }
        ...
        try {
            $response = Http::asJson()
                ...
                ->retry(2, 1000, fn ($exception, $request) =>
        ...
        if ($response->failed()) {
            ...
            report(new RuntimeException(
                "Gemini vision request failed ({$status}) for model {$this->model}: " . ($apiMessage ?: $response->body())
            ));
            ...
            throw new RuntimeException(match ($status) {
                ...
                default => 'Gemini AI could not analyze this image. Please try again.',
            });
        }
        ...
        if (! is_string($text) || trim($text) === '') {
            throw new RuntimeException('Gemini returned no usable image analysis. Please try again.');
        }
