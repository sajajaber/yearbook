...
class GraduateImportService
{
    ...
    public function validate(array $rows): array
    {
        ...
        $graduations = Graduation::with('academicYear')->get();
        ...
        foreach ($rows as $row) {
            ...
            if ($studentReference !== null) {
                ...
                if (isset($seenReferences[$key])) $duplicates[] = ['row' => $row['_line'], 'message' => 'Duplicate student reference in this file.'];
                elseif (Graduate::where('student_reference', $studentReference)->exists()) $duplicates[] = ['row' => $row['_line'], 'message' => 'Student reference already exists in the database.'];
                $seenReferences[$key] = true;
            } elseif ($school && $major && $year) {
                $naturalKey = implode('|', [$this->key($name), $school->id, $major->id, $year->id]);
                if (isset($seenNaturalKeys[$naturalKey])) $duplicates[] = ['row' => $row['_line'], 'message' => 'Duplicate graduate in this file (same name, school, major, and academic year).'];
                elseif (Graduate::where('school_id', $school->id)->where('major_id', $major->id)->where('academic_year_id', $year->id)->whereRaw('LOWER(name) = ?', [mb_strtolower($name)])->exists()) $duplicates[] = ['row' => $row['_line'], 'message' => 'A graduate with the same name, school, major, and academic year already exists.'];
                $seenNaturalKeys[$naturalKey] = true;
            }

            if ($rowErrors) {
                $errors[] = ['row' => $row['_line'], 'messages' => $rowErrors];
                continue;
            }
    ...
    public function import(array $rows): array
    {
        ...
        DB::transaction(function () use ($rows, &$created) {
            foreach ($rows as $row) {
                ...
                Graduate::create([
                    'student_reference' => $this->nullable($row['student_reference'] ?? null),
                    'name' => $row['name'],
                    'degree_level' => strtolower($row['degree_level']),
                    'school_id' => $resolved['school']->id,
                    'major_id' => $resolved['major']->id,
                    'campus_id' => $resolved['campus']->id,
                    'academic_year_id' => $resolved['year']->id,
                    'graduation_id' => $resolved['graduation']?->id,
                    'consent_status' => $resolved['consent'],
                    'publish_status' => 'draft',
                    'gpa' => $resolved['gpa'],
                    'profile_text' => $this->nullable($row['profile_text'] ?? null),
                    'achievements' => $this->list($row['achievements'] ?? ''),
                    'activities' => $this->list($row['activities'] ?? ''),
                    'projects' => $this->list($row['projects'] ?? ''),
                    'internships' => $this->list($row['internships'] ?? ''),
                    'certifications_training' => $this->nullable($row['certifications_training'] ?? null),
                    'professional_interests' => $this->nullable($row['professional_interests'] ?? null),
                    'future_plans' => $this->nullable($row['future_plans'] ?? null),
                    'quote' => $this->nullable($row['quote'] ?? null),
                ]);
                $created++;
            }
        });
    ...
    private function findByName($collection, string $value)
    {
        $needle = $this->key($value);
        return $collection->first(fn ($item) => $this->key($item->name) === $needle);
    }

    private function findYear($years, string $value)
    {
        $needle = $this->key($value);
        return $years->first(fn ($year) => $this->key($year->title) === $needle || (string) $year->id === trim($value));
    }

    private function findGraduation($graduations, string $value, ?int $yearId)
    {
        if (trim($value) === '') return null;
        return $graduations->first(function ($graduation) use ($value, $yearId) {
            return (! $yearId || $graduation->academic_year_id === $yearId)
                && ($this->key($graduation->venue) === $this->key($value) || (string) $graduation->id === trim($value));
        });
    }
