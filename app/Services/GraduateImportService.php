...
{
    public const HEADERS = ['student_reference','name','degree_level','school','major','campus','academic_year','graduation','consent_status','gpa','profile_text','achievements','activities','projects','internships','certifications_training','professional_interests','future_plans','quote'];
    ...
    public function parse(string $path): array
    {
        ...
        while (($values = fgetcsv($handle)) !== false) {
            ...
            $values = array_pad($values, count($headers), null); $row = [];
    ...
    public function validate(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $row['_resolved'] = compact('school','major','campus','year','graduation','consent','gpa'); $valid[] = $row;
        }
    ...
    public function import(array $rows): array
    {
        ...
        DB::transaction(function () use ($rows, &$created) {
            foreach ($rows as $row) {
                ...
                $created[] = Graduate::create([
                    'student_reference'=>$this->nullable($row['student_reference'] ?? null),'name'=>$row['name'],'degree_level'=>strtolower($row['degree_level']),
                    'school_id'=>$resolved['school']->id,'major_id'=>$resolved['major']->id,'campus_id'=>$resolved['campus']->id,'academic_year_id'=>$resolved['year']->id,'graduation_id'=>$resolved['graduation']?->id,
                    'consent_status'=>$resolved['consent'],'publish_status'=>'draft','gpa'=>$resolved['gpa'],'profile_text'=>$this->nullable($row['profile_text'] ?? null),
                    'achievements'=>$this->list($row['achievements'] ?? ''),'activities'=>$this->list($row['activities'] ?? ''),'projects'=>$this->list($row['projects'] ?? ''),'internships'=>$this->list($row['internships'] ?? ''),
                    'certifications_training'=>$this->nullable($row['certifications_training'] ?? null),'professional_interests'=>$this->nullable($row['professional_interests'] ?? null),'future_plans'=>$this->nullable($row['future_plans'] ?? null),'quote'=>$this->nullable($row['quote'] ?? null),
                ]);
            }
        });
