...
class PublicYearbookController extends Controller
{
    ...
    /**
     * Show the combined yearbook (events + graduations + graduates) for a
     * given yearbook year, e.g. /yearbook/26 or /yearbook/2026 both mean
     * the academic year that ends in 2026 ("yearbook26").
     */
    public function book(string $academicYear)
    {
        $yearbookYear = strlen($academicYear) <= 2
            ? (int) ('20' . str_pad($academicYear, 2, '0', STR_PAD_LEFT))
            : (int) $academicYear;

        $academicYearIds = AcademicYear::idsForYearbookYear($yearbookYear);
        ...
        $graduations = Graduation::whereIn('academic_year_id', $academicYearIds)
            ->with(['media', 'campuses', 'schools'])
            ->get();

        $graduates = Graduate::where('publish_status', 'published')
            ->where('consent_status', 'granted')
            ->whereIn('graduation_id', $graduations->pluck('id'))
            ->with(['media', 'school', 'major', 'campus'])
            ->orderBy('name')
            ->paginate(16);

        return view('public.yearbook.book', [
            'yearbookYear' => $yearbookYear,
            'events' => $events,
